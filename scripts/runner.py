...
def _box_experiment_5_20(param_pair):

    e = BoxedCavesExperiment(5, 20, param_pair[0])
    e.setup(percolation_limit=True)
    e.iterate(20, noise_level=param_pair[1])

    pp = param_pair
    save_path = os.path.join(
            'output', 'box-experiment_bw={:.2f}_nl={:.2f}'.format(pp[0], pp[1])
    )
...
def box_experiment(
    ...
    Parallel(n_jobs=cpu_count())(
        delayed(_box_experiment_5_20)(p) for p in param_pairs
    )
